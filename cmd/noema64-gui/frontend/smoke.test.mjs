...
test("bundle wires core actions and renders trace metadata", () => {
  for (const token of [
    ...
    "plyCountText(snapshot.ply)",
    "countText(summary.games_played, \"game\")",
    "plyCountText(game?.plies)",
    "No guided lesson available.",
    ...
    "keyField.focus()",
    "keyField.select?.()",
    "cloudProviderAckMessage",
    "showError(cloudProviderAckMessage, \"#settingsOutput\")",
    "clearCloudProviderAckWarning",
    "output?.textContent === cloudProviderAckMessage",
    "setAppActivity(\"Ready\", \"Noema64 is ready.\", \"ready\", false)",
    ...
    "showExportCanceled",
    "finishExport",
    "finishFineTuneExport",
    "fineTuneDatasetJSONL",
    "renderFineTuneEmptyState",
    "showFineTuneLabEmptyState",
    ...
    "TrainLocalPolicyPrior\", datasetJSONL",
...
test("dialog and control styles stay usable on narrow screens", () => {
  ...
  assert.match(stylesCSS, /@media \(max-width: 520px\) \{[\s\S]*\.toolbar-group \{[\s\S]*display: inline-flex;[\s\S]*flex: 0 0 auto;[\s\S]*border-right: 1px solid #d9dee7;/);
  assert.match(stylesCSS, /@media \(max-width: 520px\) \{[\s\S]*\.toolbar-group button \{[\s\S]*flex: 0 0 auto;[\s\S]*white-space: nowrap;/);
