...
- Provider comparison runs the same position suite across configured provider profiles. Endpoint-backed profiles such as OpenAI, OpenAI-compatible, Anthropic, Gemini, and Ollama are skipped until the cloud-provider acknowledgement is enabled, so comparison does not silently transmit game data.
