...
Mock provider mode is fully offline. Endpoint-backed provider modes may send the current FEN, legal moves, move history, strategy memory, selected settings, and optional user notes to the configured provider, including OpenAI, Anthropic, Gemini, local or remote OpenAI-compatible endpoints, and Ollama endpoints.
